...
pub fn report(problem: Problem) -> String {
    match problem {
        ...
        Problem::ReadElmi(read_elmi::Problem::NoElmiToJsonOutput) => String::from(format!(
            "`elm-interface-to-json` did not produce any output. \
        ...
        Problem::ReadElmi(read_elmi::Problem::CompilationFailed(_)) => {
            String::from("Test compilation failed.")
        }
        ...
        Problem::NoTestsFound(filenames) => if filenames.is_empty() {
            format!(
        ...
        Problem::Cli(cli::Problem::InvalidInteger(arg, flag_name)) => format!(
            "{} is not a valid value for argument for the {} flag",
            arg,
            flag_name
        ),
        Problem::Cli(cli::Problem::InvalidCompilerFlag(path_to_elm_binary)) => format!(
            "The --compiler flag must be given a valid path to an elm executable,\
             which this was not: {}",
            path_to_elm_binary
        ),
        Problem::ReadElmJson(ElmJsonError::OpenElmJson(_)) => String::from(
            "Unable to open your project's elm.json file for reading. \
             Please make sure it exists and has the right permissions!",
        ),
        Problem::ReadElmJson(ElmJsonError::ReadElmJson(_)) => String::from(
        ...
        Problem::ReadElmJson(ElmJsonError::ParseElmJson(_)) => String::from(
        ...
        Problem::ReadElmJson(ElmJsonError::InvalidSourceDirectories) => String::from(
        ...
        Problem::ReadElmJson(ElmJsonError::InvalidSourceDirectory(source_dir)) => format!(
        ...
        Problem::ExposedTest(path, exposed_tests::Problem::ParseError) => format!(
        ...
        Problem::ExposedTest(path, exposed_tests::Problem::MissingModuleDeclaration) => format!(
            "File \"{}\" needs a `module` declaration on the first line.",
            path.as_os_str().to_str().unwrap_or(""),
        ),
        Problem::ExposedTest(path, exposed_tests::Problem::OpenFileToReadExports(_)) => format!(
            "Could not open \"{}\" when attempting to validate its exports.",
            path.as_os_str().to_str().unwrap_or(""),
        ),
        Problem::ExposedTest(path, exposed_tests::Problem::ReadingFileForExports(_)) => format!(
            "Could not read \"{}\" when attempting to validate its exports.",
            path.as_os_str().to_str().unwrap_or(""),
        ),
        Problem::NoExposedTests(any_args) => if any_args {
            String::from(
